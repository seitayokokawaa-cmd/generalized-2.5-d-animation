...
def view_matrix(cam: CamState, depth: float, width: int, height: int) -> Mat:
    ...
    s = cam.zoom * ppm * k
    if abs(s) < 1e-6:                 # keep the matrix invertible
        s = 1e-6
...
class CameraTrack:
    ...
    def __init__(self, spec: CameraSpec, directions: list[Direction],
        ...
        for k in spec.keys:
            events.append((fnum(k.get("t", 0.0), 0.0), dict(k)))
        for d in directions:
            p = dict(d.params)
            over = fnum(p.pop("over", 0.0), 0.0, lo=0.0)
        ...
        for t, k in events:
            ...
            over = fnum(k.get("__over__", 0.0), 0.0, lo=0.0)
            t_end = t + over
            for field, track in (("x", self.x), ("y", self.y), ("zoom", self.zoom)):
                if field in k and isinstance(k[field], (int, float)) \
                        and not isinstance(k[field], bool):
                    value = fnum(k[field], 1.0 if field == "zoom" else 0.0)
                    if field == "zoom":
                        value = max(value, 0.05)
                    if over > 0.0:
                        track.hold(t)
                    track.add(t_end, value, ez)
    ...
    def state(self, t: float,
        ...
        if self.follow is not None and follow_pos is not None:
            ...
            off = fvec2(self.follow.get("offset"), (0.0, 1.0))
            zoom = fnum(self.follow.get("zoom"), 1.0, lo=0.05, hi=50.0)
            return CamState(x=follow_pos[0] + off[0],
                            y=follow_pos[1] + off[1],
                            zoom=zoom)
